...
public class Principal {
    public static void main(String args[])  throws IOException {
        ...
        receitahtmlLexer lex = new receitahtmlLexer(cs); //Construção do léxico.
...
//        }
        ...
        while ((t = lex.nextToken()).getType() != Token.EOF) //Loop criado para ler todos os tokens do arquivo até que o tipo do token retornado não seja fim de arquivo.
        {
            if (t.getType() == 2 || t.getType() == 3) {
                pr.println("<'" + t.getText() + "','" + t.getText() + "'>"); // Impressão do token referente a regra léxica PALAVRAS_RESERVADAS e T__1 (presente no arquivo receitahtmlLexer.tokens) no formato exigido no Trabalho 1.
            } else {
                if (t.getType() == 4 || t.getType() == 5 || t.getType() == 6 || t.getType() == 7) {
                    pr.println("<'" + t.getText() + "'," + receitahtmlLexer.VOCABULARY.getDisplayName(t.getType()) + ">"); // Impressão,  no formato exigido, do token referente a uma das regras léxicas seguintes: NUM_INT, IdentVarfuncCham, STRING, CARACTERESIGNORADOS (que não é impresso já que há um skip() no arquivo com as regras léxicas) e COMENTARIO (que também não é impresso já que há um skip() no arquivo com as regras léxicas).
                } else {
                    if (t.getType() == 9) {
                        pr.println("Linha " +t.getLine()+ ": string nao fechada."); // Impressão de um aviso que é criado a partir da detecção pela regra léxica COMENTARIOSEMBARRA de um possível comentário em que está faltando um caractere '/' no início do comentário.
                        possuiErroLexico = true;
                        System.out.println("Erro léxico identificado.");
                        break;
                    } else {
                        if (t.getType() == 8) {
                            pr.println("Linha " +t.getLine()+ ": comentario sem barra dupla."); // Impressão de um aviso que é criado a partir da detecção pela regra léxica STRINGNFECHADA de uma string não fechada com aspas duplas.
                            possuiErroLexico = true;
                            System.out.println("Erro léxico identificado.");
                            break;
                        } else {
        ...
        receitahtmlParser parser = new receitahtmlParser(tokens); //Inicialização do parser. Para sua inicialização é passado como argumento o fluxo de tokens, que aponta para o lexer, que aponta para o arquivo inserido na linha de comando.
        MeuErrorListener meuErrorListener = new MeuErrorListener(pr);
        
        //meuErrorListener.syntaxError(parser, tokens, 0, 0, "TESTE", ee);
